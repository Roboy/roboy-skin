    ...
    if Comp(j,3) == 1
        for i = 1:sum(hole_num)
            if (Comp(i,3) == 0)
                ...
                % calculate intensity
                intensity = sqrt(angle_LED)*sqrt(angle_sensor)*exp(-dis_l*material_loss)*exp(-(1/sqrt(2*pi)/press_area*exp(-(dis_g/press_area)^2/2)));
                text(Comp(i,1)-0.8,Comp(i,2)-0.2,mat2str(intensity));
...
%                 end
    ...
    for i = 1:hole_num(1) % 1 for red, means LED
        if Comp(i,3) == 1
            ...
            rectangle('Position',[Comp(i,1)-OD_size(1)/2 Comp(i,2) OD_size],'FaceColor',[0,0,Intensity(j,i)]);
        end
    end
    ...
    for i = hole_num(1)+1:sum(hole_num(1:2))
        if Comp(i,3) == 1
            ...
            rectangle('Position',[Comp(i,1)-OD_size(2) Comp(i,2)-OD_size(1)/2 OD_size],'FaceColor',[0,0,Intensity(j,i)]);
        end
    end
    ...
    for i = sum(hole_num(1:2))+1:sum(hole_num(1:3))
        if Comp(i,3) == 1
            ...
            rectangle('Position',[Comp(i,1)-OD_size(1)/2 Comp(i,2)-OD_size(2) OD_size],'FaceColor',[0,0,Intensity(j,i)]);
        end
    end
    ...
    for i = sum(hole_num(1:3))+1:sum(hole_num)
        if Comp(i,3) == 1
            ...
            rectangle('Position',[Comp(i,1) Comp(i,2)-OD_size(2)/2 OD_size],'FaceColor',[0,0,Intensity(j,i)]);
        end
    end
