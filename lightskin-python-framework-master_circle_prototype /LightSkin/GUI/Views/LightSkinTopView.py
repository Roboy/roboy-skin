...
class LightSkinTopView(tk.Canvas):
    ...
    def __init__(self, parent,
        ...
        if measurable_grid is not None:
            ...
            self.gridWidth = gdef.cellsX  # gridWidth und gridHeight are of type int
        ...
        # Configure event: reacts when the canvas object changes in size or it's position in window changes
        self.bind("<Configure>", self.on_resize)

        # self.height and self.width are initialized with 500
    ...
    # resizing doesn't work yet
    def on_resize(self, event):
        # determine the ratio of old canvas width/height to new canvas width/height
    ...
    def _draw(self):
        ...
        # later contains the middle of the min and max values of the rectangle and circular objects
        self._min_pos_x: float = math.inf
        self._min_pos_y: float = math.inf
        self._max_pos_x: float = -math.inf
        self._max_pos_y: float = -math.inf

        # later contains all the rectangle and circle objects
        self._leds = []
        self._sensors = []

        # initialisiert variablen von oben. Kreise haben einen Radius von 50. Quadrate eine Länge von 50.
        for _s in self.skin.sensors:
            s = (_s[0] * self._elScale, _s[1] * self._elScale) # warum koordinaten mal 100 nehmen?
        ...
        # Calculates width of a single reconstruction rectangle
        rect_w = float(self._max_pos_x - self._min_pos_x) / self.gridWidth
        rect_h = float(self._max_pos_y - self._min_pos_y) / self.gridHeight

        # Initializes _grid (a matrix) with rectangles
        self._grid = []
        ...
        # Moves a canvas item to the bottom of the canvas stack
