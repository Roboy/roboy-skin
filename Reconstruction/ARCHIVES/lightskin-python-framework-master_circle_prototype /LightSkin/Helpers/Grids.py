...
class ValueGridDefinition(ValueGridAreaDefinition):
    ...
    def getCellAtPoint(self, x: float, y: float, borders=True) -> Optional[Tuple[int, int]]:  # Wozu borders?
    ...
    def getIatX(self, x: float, borders=True) -> Optional[int]:
        ...
        if borders is False and (i != i_t):  # Wozu?
            return None
        return i
    ...
    def getJatY(self, y: float, borders=True) -> Optional[int]:
        ...
        if borders is False and (j != j_t):  # Wozu?
            return None
        return j
